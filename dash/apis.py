...
class ChartData(APIView):
    ...
    def get(self, request, format=None):
        data = {
            'nz_eth_data': self.nz_eth_data,
            'nz_eth_dates': self.nz_eth_dates,
            'nz_btc_data': self.nz_btc_data,
            'nz_btc_dates': self.nz_btc_dates,
            'nz_bch_data': self.nz_bch_data,
            'nz_bch_dates': self.nz_bch_dates,
        }
...
class CC_GetMarketHistory():
    ...
    def run(self):
        ...
        url += '&tsym='
        url += self.Currency
        url += '&limit=60&aggregate=1&toTs='
        url += str(time.mktime(datetime.now().timetuple()))
